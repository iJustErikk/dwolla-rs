...
impl DwollaClient {
    // for now, just get access token each time
    // a better solution would be keeping track of the access token so we don't have to keep getting it
    // the token expires after 1 hour, so just fetch a new token on any requests surpassing the last token request time + 3600
    // could do that with client middleware, but that would require keeping a client around
    // a better solution would be keeping this as state on whatever server we have
    // this would require acquiring a lock everytime a dwolla request is made
    // if we need to jackhammer dwolla with requests and can tolerate slightly more errors, an eventually consistent left-right data structure could be used in place of a lock
    pub async fn from_env() -> Self {
        Self {
            client: httpclient::Client::new()
                .base_url(
                    std::env::var("DWOLLA_URL")
                        .expect("Missing environment variable DWOLLA_URL")
                        .as_str(),
                )
                // easier than specifying bearer auth in every req
                .default_header("Authorization", &format!("Bearer {}", DwollaClient::get_access_token().await))
        }
    }
    // isolate from the autogenerated part
    pub async fn get_access_token() -> String {
        let client_id = std::env::var("DWOLLA_KEY").expect("Missing environment variable DWOLLA_KEY");
        let client_secret = std::env::var("DWOLLA_SECRET").expect("Missing environment variable DWOLLA_SECRET");
        let basic_token = base64::encode(&format!("{}:{}", client_id, client_secret));
        let params = [
        ...
        println!("bt {}", basic_token);
        let granttype = String::from("client_credentials");
        let body = httpclient::Client::new()
        .base_url(&std::env::var("DWOLLA_URL").expect("Missing environment variable DWOLLA_URL"))
        .post("token")
        .header("Authorization", &format!("Basic {}", basic_token))
        // .basic_auth(&basic_token)
        .content_type("application/x-www-form-urlencoded").
        // client_id={}&client_secret={}&grant_type=client_credentials
        body(httpclient::InMemoryBody::Text(serde_urlencoded::to_string(&params).unwrap())).send().await.unwrap().text().await.unwrap();
        // client_id=yzBDvllo9g6KHzQ3tzrgAcFOpsZri5jxXItLj8LFxtHZWDZ3Uc&client_secret=CZ1MQmRU8zMXWkmdxfSFEcWkNAfyQzqhsQydTcNtbZaj5S9DBf&grant_type=client_credentials
        // client_id=yzBDvllo9g6KHzQ3tzrgAcFOpsZri5jxXItLj8LFxtHZWDZ3Uc&client_secret=CZ1MQmRU8zMXWkmdxfSFEcWkNAfyQzqhsQydTcNtbZaj5S9DBf&grant_type=client_credentials
        println!("blah {}", serde_urlencoded::to_string(&params).unwrap());
        println!("b {}", body);
        panic!("no");
    }
}
