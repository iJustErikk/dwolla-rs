...
#[derive(Clone)]
pub struct AddLedgerEntryRequest<'a> {
    pub(crate) http_client: &'a DwollaClient,
    pub idempotency_key: Option<String>,
    pub amount: Amount,
    pub id: String,
}
impl<'a> AddLedgerEntryRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<()> {
        let mut r = self
            .http_client
            .client
            .post(&format!("/labels/{id}/ledger-entries", id = self.id));
...
impl<'a> ::std::future::IntoFuture for AddLedgerEntryRequest<'a> {
