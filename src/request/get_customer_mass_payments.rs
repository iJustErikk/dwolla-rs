...
#[derive(Clone)]
pub struct GetCustomerMassPaymentsRequest<'a> {
...
impl<'a> GetCustomerMassPaymentsRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<()> {
        let mut r = self
            .http_client
            .client
            .get(&format!("/customers/{id}/mass-payments", id = self.id));
...
impl<'a> ::std::future::IntoFuture for GetCustomerMassPaymentsRequest<'a> {
