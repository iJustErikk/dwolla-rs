...
#[derive(Clone)]
pub struct GetCustomerTransfersRequest<'a> {
...
impl<'a> GetCustomerTransfersRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<()> {
        let mut r = self
            .http_client
            .client
            .get(&format!("/customers/{id}/transfers", id = self.id));
...
impl<'a> ::std::future::IntoFuture for GetCustomerTransfersRequest<'a> {
