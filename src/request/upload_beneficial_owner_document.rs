...
#[derive(Clone)]
pub struct UploadBeneficialOwnerDocumentRequest<'a> {
    pub(crate) http_client: &'a DwollaClient,
    pub idempotency_key: Option<String>,
    pub id: String,
}
impl<'a> UploadBeneficialOwnerDocumentRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<()> {
        let mut r = self
            .http_client
            .client
            .post(&format!("/beneficial-owners/{id}/documents", id = self.id));
...
impl<'a> ::std::future::IntoFuture for UploadBeneficialOwnerDocumentRequest<'a> {
