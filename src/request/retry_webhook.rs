...
#[derive(Clone)]
pub struct RetryWebhookRequest<'a> {
    pub(crate) http_client: &'a DwollaClient,
    pub idempotency_key: Option<String>,
    pub id: String,
}
impl<'a> RetryWebhookRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<()> {
        let mut r = self
            .http_client
            .client
            .post(&format!("/webhooks/{id}/retries", id = self.id));
...
impl<'a> ::std::future::IntoFuture for RetryWebhookRequest<'a> {
