...
impl<'a> UpdateCustomerRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<()> {
        let mut r = self
            .http_client
            .client
            .post(&format!("/customers/{id}", id = self.id));
        ...
        if let Some(ref unwrapped) = self.controller {
            r = r.json(json!({ "controller" : unwrapped }));
        }
    ...
    pub fn controller(mut self, controller: serde_json::Value) -> Self {
