...
#[derive(Clone)]
pub struct GetCustomerFundingSourcesRequest<'a> {
    pub(crate) http_client: &'a DwollaClient,
    pub id: String,
    pub removed: Option<bool>,
}
impl<'a> GetCustomerFundingSourcesRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<()> {
        let mut r = self
            .http_client
            .client
            .get(&format!("/customers/{id}/funding-sources", id = self.id));
...
impl<'a> ::std::future::IntoFuture for GetCustomerFundingSourcesRequest<'a> {
