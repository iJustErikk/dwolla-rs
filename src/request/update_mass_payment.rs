...
#[derive(Clone)]
pub struct UpdateMassPaymentRequest<'a> {
    pub(crate) http_client: &'a DwollaClient,
    pub idempotency_key: Option<String>,
    pub id: String,
    pub status: String,
}
impl<'a> UpdateMassPaymentRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<()> {
        let mut r = self
            .http_client
            .client
            .post(&format!("/mass-payments/{id}", id = self.id));
...
impl<'a> ::std::future::IntoFuture for UpdateMassPaymentRequest<'a> {
