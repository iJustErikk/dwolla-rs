...
#[derive(Clone)]
pub struct CreateAccountFundingSourceRequest<'a> {
...
impl<'a> CreateAccountFundingSourceRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<()> {
        let mut r = self.http_client.client.post("/funding-sources");
...
impl<'a> ::std::future::IntoFuture for CreateAccountFundingSourceRequest<'a> {
